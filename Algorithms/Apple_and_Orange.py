...
fruit. Using the information given below, determine the number of apples 
...
tree of origin along the x-axis. *A negative value of d means the fruit fell  
d units to the tree's left, and a positive value of  means it falls  units to 
the tree's right. *
...
position of the tree, they land at [4+2,4+3,4+(-4)] = [6,7,0]. Oranges land 
at [12+3,12+(-2),12+(-4)] = [15,10,8]. One apple and two oranges land in 
the inclusive range 7-10 so we print:
...
should print the number of apples and oranges that land on Sam's house, 
...
The fourth line contains m space-separated integers denoting the
respective distances that each apple falls from point a.
The fifth line contains n space-separated integers denoting the 
respective distances that each orange falls from point b.
